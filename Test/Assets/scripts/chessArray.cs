...
public class chessArray : MonoBehaviour{
    ...
    List<(int,int)> showPossibleMovesArray(GameObject piece){
        ...
        resetBoard();
        if (moveCache.ContainsKey(piece)){
            
            coups = moveCache[piece];
        }
        else{
    ...
    List<(int,int)> getPossibleDiagonalPawnMovesArray(int i, int j){
    ...
    List<(int,int)> isPinnedArray(GameObject piece){ // returns a list of coordinates the pinned piece can go to while still be pinned or eat the piece
